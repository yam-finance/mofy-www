...
<div class="colc-item hover:bg-gray hover:bg-opacity-20">
	<a href={`/nft/${nft.id}`} class="block p-8 sm:px-0">
		<div class="group block w-full focus-within:ring-2 focus-within:ring-offset-2 focus-within:ring-offset-gray-100 focus-within:ring-gray-500 overflow-hidden">
			{#if !nftImage}
			...
			<button type="button" on:click={() => goto(`/nft/${nft.id}`)} class="text-black dark:text-white absolute inset-0 focus:outline-none">
		...
		<div class="flex items-start border-b border-dashed border-black dark:border-white mt-2 py-2">
			<div class="line leading-tight flex-grow">
				{#if !metadata}
				<p class="my-0 block text-black dark:text-white opacity-50 truncate pointer-events-none"> loading Name ...</p>
				{:else}
				<p class="my-0 block text-black dark:text-white truncate pointer-events-none">{metadata.name}</p>
				{/if}
				<p class="block text-xs mb-2 font-medium text-black dark:text-white opacity-50">By {ethers.utils.getAddress(nft.creatorAddress).substring(0, 8)}</p>
			</div>
			<div class="w-5 h-5 border-r-2 border-t-2 border-black dark:border-white flex-shrink-0">
				<div class="ml-0.5 -mt-0.5">
					<div class="w-5 border-t border-b border-black dark:border-white -rotate-45 translate-y-2"></div>
				</div>
			</div>
		</div>
	</a>	
